...
with open('js/commentSystem.js', 'w', encoding='utf-8') as f:
    f.write(comment_js_content)

print("SUCCESS: js/commentSystem.js generated!")
os._exit(0)



pattern = r'#*\s*\\*#\\*#\\*#\s*(\d{1,2})\\*\.\s*([^\n]+)'
...
for i, m in enumerate(matches):
    ...
    for line in section_text.splitlines():
        ...
        if not s or s.startswith('-문헌') or s.startswith('-과학해설') or s.startswith('###') or s.startswith('#'):
            continue
        if s.startswith('http') or 'skfb.ly' in s or 'digital.khs' in s or 'sketchfab.com' in s:
            assets.append(s)
        elif len(s) > 8 and not s.startswith('-3D'):
            papers.append(s)
            
    animal_data[num] = {'name': name, 'papers': papers, 'assets': assets}

with open('js/data.js', 'r', encoding='utf-8') as f:
    data_js = f.read()

for a_id in range(1, 20):
    code_str = f"{a_id:02d}"
    refs = animal_data.get(a_id, {'papers': [], 'assets': []})
    
    # 4개 대표 논문/서지정보
    r_items = []
    for idx, p in enumerate(refs['papers'][:5], start=1):
        clean_p = p.replace("'", "\\'").replace('"', '\\"')
        r_items.append(f"{{ code: 'R{code_str}-{idx:02d}', text: '{clean_p}' }}")
    if not r_items:
        r_items.append(f"{{ code: 'R{code_str}-01', text: '국립부여박물관 백제금동대향로 자연사 및 문화생태 연구' }}")
        
    # 에셋 출처
    a_items = []
    for idx, a in enumerate(refs['assets'][:2], start=1):
        clean_a = a.replace("'", "\\'").replace('"', '\\"')
        a_items.append(f"{{ code: 'A{code_str}-{idx:02d}', text: '{clean_a}' }}")
    if not a_items:
        a_items.append(f"{{ code: 'A{code_str}-01', text: '국가유산 디지털 서비스 및 3D 모델 데이터베이스' }}")

    r_str = ",\n        ".join(r_items)
    a_str = ",\n        ".join(a_items)

    inject_code = f"""      sourceCode: 'R{code_str}',
      referenceList: [
        {r_str}
      ],
      assetList: [
    ...
    # Replace existing sourceCode/referenceList block
    sub_pattern = rf"(code:\s*'{code_str}'.*?)(sourceCode:\s*'[^']+',\s*referenceList:\s*\[.*?assetList:\s*\[.*?\])"
    
    def replacer(match):
        return match.group(1) + inject_code
        
    data_js = re.sub(sub_pattern, replacer, data_js, flags=re.DOTALL)

with open('js/data.js', 'w', encoding='utf-8') as f:
    f.write(data_js)

print("SUCCESS: 19 Animals Rich R/A Citation Datasets Generated!")
os._exit(0)
